    ...
    def add_dyes(self, x: int, y: int, drops: List[int]) -> bool:
        ...
        for dye_idx, drop_count in enumerate(drops):
            if drop_count > 0:
                ...
                self.plate[x, y] += np.array(self.dyes[dye_idx]) * (drop_count / sum(drops))
...
def add_dyes(x: int, y: int):
    ...
    if 'drops' not in data or len(data['drops']) != 3:
        return jsonify({'error': 'Invalid drops data. Must provide array of 3 integers.'}), 400
...
def get_well_color(x: int, y: int):
    ...
    return jsonify({
        'color': color  # Returns hex color string like "#ff0000"
    })
